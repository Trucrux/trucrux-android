...
function add_imx8mm_menu() {
	img_list+=("dtbo-imx8mm-trux-q01-8MDVP.img           (TRUX-MX8M-MINI on 8MDVP 2.x and above)")
}

function add_imx8mq_menu() {
	img_list+=("dtbo-imx8mq-trux-q01-8MDVP-wifi-hdmi.img                (TRUX-MX8M on 8MDVP 2.x and above, with HDMI and WiFi support)")
	img_list+=("dtbo-imx8mq-trux-q01-8MDVP-wifi-mipi.img                (TRUX-MX8M on 8MDVP 2.x and above, with MIPI and WiFi support)")
}
...
if [[ $soc_name == "showoptions" ]] && [[ ${#img_list[@]} > 1 ]] ; then
	...
	do
		if [[ -z "$opt" ]] ; then
			...
			if grep -q "i.MX8MM\|i.MX8MQ" /sys/devices/soc0/soc_id; then
...
function create_parts
{
	...
	sgdisk -n 1:16384:+${DTBO_ROM_SIZE}M		-c 1:"dtbo_a"      -t 1:8300  $node
